...
class InsertDataset(Dataset):
    def __init__(
        ...
        assert (action_1_str, action_2_str) in [(x.action_1_str, x.action_2_str) for x in [InsertLeftEndImiModule, InsertRightEndImiModule]], (action_1_str, action_2_str)
...
class DualArmImiModule(pl.LightningModule):
    action_1_str: str
    action_2_str: str
    ...
    def preprocess_input_2(self, depth: torch.Tensor, mask: torch.Tensor):
        """
        Args:
        - depth: [B, H, W]
        - mask: [B, 3, H, W]
    ...
    def extract_data(self, batch):
        ...
        return depth, mask, action_1_ij.to(torch.long), action_2_ij.to(torch.long), action_1_space, action_2_space
    ...
    def imi_loss(self, dense: torch.Tensor, action: torch.Tensor):
        B, H, W = dense.shape
        loss = torch.nn.functional.cross_entropy(dense.view(B, -1), action[:, 0] * W + action[:, 1])
        return loss
    
    def forward_all(self, batch):
        depth, mask, action_1_ij, action_2_ij, action_1_space, action_2_space = self.extract_data(batch)

        i_1 = self.preprocess_input_1(depth, mask)
        i_2 = self.preprocess_input_2(depth, mask)
        dense_1, dense_2 = self.run_forward(i_1=i_1, i_2=i_2)

        loss_1 = self.imi_loss(dense_1, action_1_ij)
        loss_2 = self.imi_loss(dense_2, action_2_ij)
        ...
        return loss_1, loss_2, dense_info
    
    def training_step(self, batch, batch_idx):
        opt1, opt2 = self.optimizers()
        sch1, sch2 = self.lr_schedulers()

        loss_1, loss_2, dense_info = self.forward_all(batch)
        self.log_all(loss_1, loss_2, "train")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx > 0:
            self.log_all(loss_1, loss_2, "valid")
        
        if batch_idx in self.learn_kwargs["valid"]["plot_batch_idx"]:
            self.log_img(batch_idx, dense_info)
    
    def log_all(self, loss_1: torch.Tensor, loss_2: torch.Tensor, name: str):
...
class InsertLeftEndImiModule(DualArmImiModule):
    action_1_str = "press"
    action_2_str = "lift"
...
class InsertRightEndImiModule(DualArmImiModule):
    action_1_str = "drag"
    action_2_str = "rotate"
...
class InsertPolicyImi(InsertPolicy):
