...
from robohang.policy.insert.insert_learn_imi import InsertPolicyImi
...
def main(cfg: omegaconf.DictConfig):
    ...
    insert_policy = InsertPolicyImi(insert_gym, cfg.insert_policy)
    ...
    for traj_idx in tqdm.tqdm(range(output_cfg.total_traj), dynamic_ncols=True):
        ...
        action_and_info = insert_policy.get_press_action(info=dict(log_path=os.path.join(exporter.get_trajectory_path(), "pred", "left")))
        export_all(action_and_info)
        insert_gym.primitive_press(**(action_and_info["action"]), callbacks=callbacks)

        action_and_info = insert_policy.get_lift_action(info=dict())
        export_all(action_and_info)
        insert_gym.primitive_lift(**(action_and_info["action"]), callbacks=callbacks)

        action_and_info = insert_policy.get_drag_action(info=dict(log_path=os.path.join(exporter.get_trajectory_path(), "pred", "right")))
        export_all(action_and_info)
        insert_gym.primitive_drag(**(action_and_info["action"]), callbacks=callbacks)

        action_and_info = insert_policy.get_rotate_action(info=dict())
        export_all(action_and_info)
