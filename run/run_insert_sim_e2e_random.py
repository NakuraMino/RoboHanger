...
def main(cfg: omegaconf.DictConfig):
    ...
    insert_policy = InsertPolicyRandom(insert_gym, cfg.insert_policy)
    ...
    for traj_idx in tqdm.tqdm(range(output_cfg.total_traj), dynamic_ncols=True):
        ...
        # use heuristic to generate trajectory
        action_and_info = insert_policy.get_press_action()
        export_all(action_and_info)
        insert_gym.primitive_press(**(action_and_info["action"]), callbacks=callbacks + [exp_e2e.callback])

        action_and_info = insert_policy.get_lift_action()
        export_all(action_and_info)
        insert_gym.primitive_lift(**(action_and_info["action"]), callbacks=callbacks + [exp_e2e.callback])

        action_and_info = insert_policy.get_drag_action()
        export_all(action_and_info)
        insert_gym.primitive_drag(**(action_and_info["action"]), callbacks=callbacks + [exp_e2e.callback])

        action_and_info = insert_policy.get_rotate_action()
        export_all(action_and_info)
        insert_gym.primitive_rotate(**(action_and_info["action"]), callbacks=callbacks + [exp_e2e.callback])
